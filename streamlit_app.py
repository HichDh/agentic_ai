...
from src.agentic_ai.lc_pipeline import build_rag_chain, build_vectorstore
...
with st.sidebar:
    ...
    use_sampling = st.checkbox(
        "Use sampling", value=default_do_sample, key="use_sampling"
    )
    temperature = st.slider(
        "Temperature", 0.1, 1.5, default_temp, 0.1, key="temperature"
    )
    max_new = st.slider(
        "Max new tokens", 64, 512, default_max_new, 32, key="max_new_tokens"
    )
    ...
    strict_mode = st.checkbox(
        "Strict mode", value=True, key="strict_mode"
    )  # (does not use sources if below threshold)
...
if st.button("Ask"):
    with st.spinner("Thinking..."):
        ...
        rag_chain = build_rag_chain(
            ...
            strict=bool(st.session_state["strict_mode"]),
        )
        result = rag_chain.invoke(
            {"question": q, "chat_history": st.session_state.get("chat_history", [])}
        )
